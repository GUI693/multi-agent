...
def check_denotation(target_values, predicted_values):
    ...
    if len(target_values) != len(predicted_values):
        return False
    # Check items
    extracted_target = str(target_values)
    extracted_pred = str(predicted_values)
    if extract(extracted_target) in extract(extracted_pred):
        return True
    return False
    # for target in target_values:
    #     extracted_target = str(target)
    #     print(extract(target))
    #     if not any(extracted_target in extract(str(pred)) for pred in predicted_values):
    #         return False
    # return True
